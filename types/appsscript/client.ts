...
import type { FlowData, FlowGraphData, FlowMeta, FlowVersionDetail, Folder, Role } from '../flow';
...
export type ServerScripts = {
   ...
   * フォルダ権限取得
   */
  getFolderPermissions(folderId: string): JsonString<(PermissionRow & { avatarUrl?: string; })[]>;

  /**
   * フォルダ権限追加
   */
  addFolderPermission(folderId: string, email: string, role: Role): JsonString<PermissionRow>;

  /**
   * フォルダ権限削除
   */
  removeFolderPermission(permissionId: string): JsonString<{ success: boolean; permissionId: string; }>;

  /**
   * フォルダ権限更新
   */
  updateFolderPermission(permissionId: string, role: Role): JsonString<PermissionRow>;
